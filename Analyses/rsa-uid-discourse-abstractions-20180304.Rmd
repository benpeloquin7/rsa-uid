...
df_agg %>%
  ...
  ggplot(aes(x=utteranceNum, y=xHere)) +
    geom_jitter(alpha=0.75, height=0.05, width=0, aes(col=utterance), size=2) +
    geom_line(aes(x=utteranceNum, y=T1)) +
    theme_few() +
    facet_grid(~resultType)
```
...
binSize=20
df_binned <- addUtteranceBins(sims, nUtterances=NUM_UTTERANCES, binSize=20)
