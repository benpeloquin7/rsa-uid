---
title: "Rsa-uid-corpus-generation-20180112"
author: "Ben"
date: "1/12/2018"
...
fPath <- '/Users/benpeloquin/Desktop/Projects/rsa_uid/Models/rsa-uid-corpus-generation-20180112.wppl'
...
nSamples <- 1000
runExp1 <- runExperimentFn(runFn, 'S1', 2, 1, 0.8, nSamples)
```
...
```{r}
df <- data.frame(
  world=c(rep('a', 4), rep('b', 4)), 
  utt=rep(c('X a', 'a', 'X b', 'b'), 2),
  postProbs=c(0.46, 0.427, 0.099, 0.0077, 0.04961328492428481, 	9.661590951810712e-7, 0.5300247504353464, 0.4203609984812739),
  priorProbs=c(rep(0.2, 4), rep(0.8, 4)),
  n=rep(100, 8))
```
...
```{r}
df %>%
  mutate(cnt=postProbs*n*priorProbs) %>%
  group_by(utt) %>%
