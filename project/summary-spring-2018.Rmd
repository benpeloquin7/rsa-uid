...
df_filled %>%
  ...
  summarise(cnt=sum(n),
            prop=cnt/nSims) %>%
  group_by(binVal) %>%
  summarise(ent=entropy::entropy(prop, unit=c('log2')),
                            # a  , b,   c,   d,  x,     y
            MI=getMI(prop, c(0.2, 0.15, 0.2, 0.1, 0.25, 0.1)),
            diff=ent-MI) %>%
  ggplot(aes(x=binVal, y=ent)) +
    geom_line(col='blue', size=1.5, alpha=0.6) +
    # geom_line(aes(y=MI), col='red', size=1.5, alpha=0.6) +
    # geom_hline(aes(yintercept=mean(diff))) +
    # geom_line(aes(y=diff), col='grey', size=1.5, alpha=0.6, lty=2) +
    ylab("Entropy") +
    xlab("Utterance position") +
    ggtitle("Uncondtional entropy over discourse") +
    theme_few() +
    theme(plot.title=element_text(hjust = 0.5))
```
