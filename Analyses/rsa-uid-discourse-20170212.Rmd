...
# How does optimal ordering vary with alpha
...
dfs <- list(df_optimalS2_16, df_optimalS2_8, df_optimalS2_2)
res_df <- data.frame()
for (df in dfs) {
  N <- 50
  firstN <- as.list(df %>%
...
df_bs_contextAwareS2 %>%
  group_by(endPoint) %>%
  summarise(avgUncondEntropy=median(uncondEntropy),
            avgMI=median(MI),
            avgHY=median(hY))
```
