...
listener_run_fn_ <- function(modelFile) {
  rData <- data.frame(alpha=1, lambda=10, modelName='L1', theta=THETA)
...
speaker_run_fn_ <- function(modelFile){
  noisy_rsa1_speaker_run_fn_ <- function(modelFile, input) {
    rData <- data.frame(alpha=5, lambda=1, modelName='S3', theta=THETA, speakerInput=input)
    rwebppl::webppl(modelFile, data=rData, data_var='rData')
  }
...
corpus_run_fn <- function(modelFile){
  runFn <- createRunFn(modelFile)
  nUtts <- 1000
  runExp_ <- runExperimentFn_(runFn, 'S3', 5, 1, THETA, nUtts)
  # Register cores
  no_cores <- detectCores() - 1
  cl <- makeCluster(no_cores, type='FORK')
  registerDoParallel(cl)
  ...
  ptm <- proc.time()
  nSims <- 100
  cat("Running", nSims, "simulations each with", nUtts, "samples.")
...
```{r}
corpus_run_fn_(getModelFile(basic_rsa1_corpus_file)) %>%
