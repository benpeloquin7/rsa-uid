...
sims <- foreach(i=seq(1, nSims), .packages=c('dplyr', 'rwebppl'), .combine=rbind) %dopar% 
  runFn(i, targetDistr='T1', nUtterances=NUM_UTTERANCES, resultType='contextAwareS4', alpha=ALPHA)
stopCluster(cl)
etm <- proc.time() - ptm
...
```{r plot-sims}
sims %>%
  mutate(`P(Topic|Utterances)`=ifelse(utterance=='x' | utterance=='X x', 0.5, ifelse(utterance=='y' | utterance=='X y', 0.75, 0)),
