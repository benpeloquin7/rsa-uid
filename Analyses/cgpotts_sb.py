    ...
    def get_metadata(self):
        ...
        with open(self.metadata_filename) as f:
            ...
            for row in csvreader:
                ...
                for key in ('conversation_no', 'length', 'from_caller_education',
                            'to_caller_education'):
                    d[key] = int(d[key])
                for key in (
                'talk_day', 'from_caller_birth_year', 'to_caller_birth_year'):
...
class Utterance:
    ...
    header = [
        'swda_filename',  # (str) The filename: directory/basename
        'ptb_basename',
        # (str) The Treebank filename: add ".pos" for POS and ".mrg" for trees
        'conversation_no',
        # (int) The conversation Id, to key into the metadata database.
        'transcript_index',
        # (int) The line number of this item in the transcript (counting only utt lines).
        'act_tag',
        # (list of str) The Dialog Act Tags (separated by ||| in the file).
        'caller',  # (str) A, B, @A, @B, @@A, @@B
        'utterance_index',
        # (int) The encoded index of the utterance (the number in A.49, B.27, etc.)
        'subutterance_index',
        # (int) Utterances can be broken across line. This gives the internal position.
        'text',  # (str) The text of the utterance
        'pos',
        # (str) The POS tagged version of the utterance, from PtbBasename+.pos
        'trees',
        # (list of nltk.tree.Tree) The tree(s) containing this utterance (separated by ||| in the file).
        'ptb_treenumbers'
    ...
    def __init__(self, row, transcript_metadata):
        ...
        for i in range(len(Utterance.header)):
            ...
            'conversation_no', 'transcript_index', 'utterance_index',
            'subutterance_index'):
    ...
    def regularize_pos_lemmas(self):
        ...
        nontree_nodes = (
        '^PRP^BES', '^FW', '^MD', '^MD^RB', '^PRP^VBZ', '^WP$', '^NN^HVS',
